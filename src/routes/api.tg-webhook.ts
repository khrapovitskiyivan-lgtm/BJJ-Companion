...
const GUIDE_URL = APP_URL + "/BJJ-Companion-guide.pdf?v=9";
...
const WELCOME = [
  ...
  "",
  "Отмечай тренировки в дневнике, смотри свой стиль и прогресс, разбирай ситуации и получай рекомендации, что учить дальше. База: 340 приёмов и позиций от белого до коричневого пояса.",
  "",
  COMMANDS_HINT,
  "",
...
const HELP = [
  ...
  "Отработка — готовый комплекс по профилю или по дневнику, с таймером и звуком.",
...
const ABOUT = [
  ...
  "",
  "Логируешь тренировки — приложение видит твой стиль, считает план и подсказывает, что учить и что повторить. 340 приёмов и позиций от белого до коричневого пояса, готовые отработки с таймером, разбор ситуаций.",
  "",
...
const REPLIES: Record<string, { text: string; reply_markup: unknown }> = {
  ...
  "/train": {
    text: "Готовая отработка уже собрана — открой и жми Старт.",
    reply_markup: appButton("Открыть отработку", "/workout"),
  },
  "/diary": {
