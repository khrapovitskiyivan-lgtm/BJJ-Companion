-- Расширение белого списка телеметрии: workout_theme
-- (показ темы-кластера отработки: detail = anchorId, дедуп раз в сутки на клиенте).
...
-- Какие темы отработки показываются чаще (какие якоря):
-- select detail, count(*) from public.bjj_events
-- where event = 'workout_theme' group by detail order by count(*) desc;
