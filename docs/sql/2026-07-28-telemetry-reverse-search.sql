-- Расширение белого списка телеметрии: reverse_search
-- (обратный поиск «опиши, что случилось»: detail = scenario:region/position,
--  например submission:neck / control:13 / takedown:-). Выполнить один раз в
--  Supabase -> SQL Editor. Полный текущий whitelist + новое событие.
...
-- Что ищут обратным поиском (какие сценарии/регионы):
-- select detail, count(*) from public.bjj_events
-- where event = 'reverse_search' group by detail order by count(*) desc;
